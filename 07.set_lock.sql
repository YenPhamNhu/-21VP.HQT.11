...
BEGIN
    BEGIN TRANSACTION;
    -- Bắt đầu giao dịch
    -- Xin khóa đọc trên bảng BENHNHAN, LICHSUKHAMBENH và trả khóa sau khi đọc xong
    -- B1: Kiểm tra bệnh nhân có tồn tại trong hệ thống không
    IF EXISTS (SELECT 1
    FROM BENHNHAN
    WHERE SDT = @SDT)
    BEGIN
        -- B2: Kiểm tra xem lịch sử khám bệnh có tồn tại không
        IF EXISTS (SELECT 1
        FROM LICHSUKHAMBENH
        WHERE STT = @STTLichSuKB AND MaBenhNhan = (SELECT MaBenhNhan
            ...
            WHERE SDT = @SDT))
        BEGIN
            -- B3: Cập nhật trạng thái thanh toán của hóa đơn
            SET TRANSACTION ISOLATION LEVEL READ COMMITTED
            UPDATE HOADON
